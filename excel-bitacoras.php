    ...
    foreach ($res as $row) {
        ...
        for ($i = 2; $i <= $column_number; $i++) {
            if (!empty($row['gestion_fecha' . $i])) {
                ...
                $bitacoras[] = [$row['acreditado_folio'], $row['acreditado_folio'], $row['acreditado_folio'], $row['acreditado_folio'], '', '', '', '', '', '', '', $row['gestion_fecha' . $i], $row['gestion_via' . $i], $row['gestion_comentarios' . $i], $row['evidencia_fecha' . $i], $row['evidencia_fotografia' . $i]];
            }
        }
    }
}
